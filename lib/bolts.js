...
function loadDefaults(opts, defaults, callback) {
	...
	['name','email'].forEach(function(type) {
		funcs.push(function(cb) {
			exec('git config --get user.' + type, { env: process.env }, function(err, stdout, stderr) {
...
function loadConfig(opts, defaults, callback) {
	var isExplicit = !!opts.config,
		config = opts.config || findConfig();

	if (config) {
		fs.readFile(config, function(err, data) {
			if (err && isExplicit) {
				if (err.code === 'ENOENT') {
					err.message = 'config file does not exist at "' + config + '"';
				}
				return callback(err);
			}
			try {
				return callback(null, opts, _.extend(defaults, err ? {} : JSON.parse(data)));
			} catch (e) {
				return callback(e);
			}
		});
	} else {
		// no config file, pass thru
		return callback(null, opts, defaults);
	}
}
...
function writeConfig(opts, callback) {
	if (!HOME) { return callback(null, opts); }
...
function generateProject(opts, callback) {
	...
	fs.renameSync(path.join(dest, '_test'), path.join(dest, 'test'));
	fs.renameSync(path.join(dest, 'module.js'), path.join(dest, opts.project + '.js'));
	fs.renameSync(path.join(dest, 'test', 'module_test.js'), path.join(dest, 'test', opts.project + '_test.js'));
	
	// rename vs.net project files
	fs.renameSync(path.join(dest, 'module.sln'), path.join(dest, opts.project + '.sln'));
	fs.renameSync(path.join(dest, 'module.njsproj'), path.join(dest, opts.project + '.njsproj'));
...
function findConfig() {
	...
	if (HOME) { configs.push(path.join(HOME, CONFIG)); }
